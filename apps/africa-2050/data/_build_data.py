...
def build_countries() -> None:
    ...
    Why hybrid (WB level + WUP 2025 trajectory):
        ...
        So we hybridise: World Bank SP.URB.TOTL.IN.ZS (which is itself derived from
        WUP national definitions, and is the audience-canonical series) sets the
        2024-actual urban rate as the 2025 baseline. UN WUP 2025's projected 10-year
        urban-population growth *ratio* (DEGURBA, 2025→2035) provides the trajectory
        — methodology-neutral as a ratio. Total-population denominators come from
        WUP 2025 F01 "Total" sheet.

        Earlier attempt used a linear WB 2014→2024 trend extrapolation, but that
        picked up historical reclassification jumps (e.g. Uganda 11%→31% from
        2000→2024) and over-projected 2035 rates for fast-urbanisers.
    """
    # World Bank: latest actual urban rate (2024 → labelled 2025).
    rate_pct = _wb_indicator("SP.URB.TOTL.IN.ZS", (2024, 2024))

    # WUP 2025 F01 supplies total-population denominators AND the trajectory.
    def _wup_by_iso(sheet: str) -> dict[str, dict]:
        header, rows = _load_sheet(
            WUP / "WUP2025-F01-Degree-of-Urbanization_Pop_by_category.xlsx", sheet
        )
        ...
        for r in rows:
            ...
            out[iso] = {
                "name": r[i_name],
                "v25": r[i_b] if isinstance(r[i_b], (int, float)) else 0.0,
                "v35": r[i_p] if isinstance(r[i_p], (int, float)) else 0.0,
            }
        return out

    total_pop = _wup_by_iso("Total")
    wup_urban = _wup_by_iso("Cities and Towns")  # DEGURBA urban — used only for growth ratio

    countries: list[dict] = []
    for iso in sorted(SOVEREIGN_ISO):
        tp = total_pop.get(iso)
        rs = rate_pct.get(iso)
        wu = wup_urban.get(iso)
        if not tp or not rs or 2024 not in rs or not wu:
            print(f"  WARN: missing data for {iso}")
            continue
        r25 = rs[2024] / 100.0
        # UN's projected 10-year urban-pop growth ratio (DEGURBA, methodology-neutral as a ratio).
        # Apply that ratio to the WB-anchored urban population at year 0.
        growth_ratio = wu["v35"] / wu["v25"] if wu["v25"] > 0 else 1.0
        urban_25 = tp["v25"] * r25
        urban_35 = urban_25 * growth_ratio
        r35 = max(0.0, min(1.0, urban_35 / tp["v35"])) if tp["v35"] else r25
        countries.append({
            "iso3": iso,
            "name": tp["name"],
            "urban_pop_2025": round(urban_25, 0),
            "urban_pop_2035": round(urban_35, 0),
            "total_pop_2025": round(tp["v25"], 0),
            "total_pop_2035": round(tp["v35"], 0),
    ...
    js = (
        ...
        "// 2035 urban pop: WB urban pop 2025 scaled by UN WUP 2025's projected\n"
        "//            10-year urban-pop growth ratio (Cities & Towns sheet).\n"
